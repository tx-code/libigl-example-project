...
namespace imgui {
void SelectionWidget::init(igl::opengl::glfw::Viewer *_viewer,
                           ImGuiPlugin *_plugin) {
...
void SelectionWidget::draw() {
  ...
  for (int pass = 0; pass < 2; pass++) {
    ...
    const bool closed =
        !(mode == LASSO || mode == POLYGONAL_LASSO) || !(is_down || is_drawing);
    if (pass == 0) {
...
bool SelectionWidget::mouse_down(int /*button*/, int modifier) {
  if (mode == OFF || (modifier & IGL_MOD_ALT)) {
    return false;
  }
  is_down = true;
...
bool SelectionWidget::mouse_move(int /*mouse_x*/, int /*mouse_y*/) {
  ...
  if (!has_moved_since_down) {
    if (mode == POLYGONAL_LASSO) {
      L.emplace_back(L[L.size() - 1]);
    }
...
bool SelectionWidget::key_pressed(unsigned int key, int /*modifiers*/) {
  Mode old = mode;
  ...
  if (old != mode) {
    clear();
    if (callback_post_mode_change) {
      callback_post_mode_change(old);
    }
    return true;
  }
  return false;
...
void SelectionWidget::circle(const Eigen::Matrix<float, 2, 2> &M,
...
void SelectionWidget::rect(const Eigen::Matrix<float, 2, 2> &M,
                           std::vector<Eigen::RowVector2f> &L) {
  L.resize(4);
...
Eigen::RowVector2f SelectionWidget::xy(const igl::opengl::glfw::Viewer *vr) {
  return Eigen::RowVector2f(vr->current_mouse_x,
